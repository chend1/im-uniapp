...
// 画头像
export function drawAvatar(imgList, callBack) {
	...
	imgList.forEach((imgUrl, index) => {
		...
		img.onload = () => {
			num++
			context && context.drawImage(img, 50 * (index % step), 50 * (parseInt(index / step + '')), 50,
				50)
			if (num === sub) {
...
// 获取会话列表提示消息
export function getPointMsg(msgType = 1, message) {
	if (msgType === 1) {
		return message
	}
	if (msgType === 3) {
